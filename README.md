...
unnoticed since Phase 40 and was fixed with it — and was itself retired in
Phase 122, leaving `parties.shared_addresses` the only counting check.
...
### The count that spanned every company (Phase 102)

The operations page builds its data in one `Promise.all`, and every call in it
takes the actor's company — `queueCounts`, `listJobs`, `listSchedules`,
`listEvents`, `health`. One took nothing. `retentionReport()` had no
`companyId` parameter at all, and seven of the eleven swept tables carry a
`company_id`, so a `company:manage` holder at one company read how many letters,
campaign events, lead submissions, relayed events and nightly check runs **every
other company on the deployment** was holding.
...
True, and answering the wrong question. It reasons about *which role* may see
the numbers and never about *whose rows they are*. Permission and tenancy are
different checks, and a permission check reads like a complete one. It survived
because an aggregate does not look like data — but "how many invoices did they
send last year" is a business fact, and the tenancy rule this codebase asserts
on every other query has no aggregate exemption.

**The report takes an audience; the sweep deliberately does not.** That
asymmetry is the safety of the change. "Scope it like everything else" applied
to `sweepAll` would be a serious bug: retention would then only delete rows
belonging to whoever last loaded a page. Deleting is housekeeping that runs
nightly as nobody; reporting is a screen somebody is looking at.

**Three kinds of table, because the schema has three.** Checking the database
before writing the code is what turned the third one up:
...
That third row is not a modelling accident. A `password_reset` token carries a
`userId` and no company, because when it is issued nobody knows which company
the address belongs to; a `practice_invitation` carries a `practiceId`, and a
firm is not a company; and the Phase 88 morning brief writes a
`transactional_messages` row whose audience is a practice. The development
database has one sitting there, which is why its eleven letters attribute to
ten. A naive `where company_id = $1` would have closed the leak and opened a
quieter hole — those rows becoming invisible to *everybody*, dropping out of the
one screen that answers "what do you hold and for how long".

**The policy is published; the count is tenant data.** A policy entry was two
things stapled together. How long and why is a statement about the product —
the retention list exists precisely so somebody can be shown it — and the row
count is the half that leaks. So every viewer still sees every policy and every
reason, and a number appears only where it can honestly be attributed. Where it
cannot, the screen gives the reason rather than a blank.

**This tripwire is derived, and Phase 101's could not be.** Whether a table has
a `company_id`, and whether it is nullable, is a fact `information_schema`
holds — so each declaration is checked against the catalogue the way Phase 96
checks `PARTY_REFERENCES` against `pg_constraint`, rather than trusted. The
valuable direction is the second: making a column nullable is a migration nobody
...
sweep pointed somewhere else would have made that guarantee vacuous while still
reading as though it held.

### The money you hold for somebody else (Phase 104)

The export answered two of the three questions a set of books has to answer
about money and people. **What are we owed** — `invoices.csv`. **What do we owe
suppliers** — `bills.csv`. **What are we holding that is somebody else's** —
nothing.

That third one is not smaller. A retainer is a customer's money sitting in the
company's account against work not yet done; a credit note with a balance is a
promise to a named customer; a gift card is a promise to whoever holds the code.
The ledger showed them as a lump — `journal.csv` with a liability account and a
balance on it — and the trail stopped there. A leaving company got *"Customer
retainers: 12,400"* and no way to find out whose. **You cannot honour a gift card
from a trial balance.**

So `retainers.csv`, `credit_notes.csv` and `gift_cards.csv`, currency-aware on
Phase 103's rules. `refunds` is deliberately *not* added: a refund is a
completed act rather than a standing obligation, the money has gone, and both
the journal and `payments.csv` already carry it — a third copy would only invite
somebody to count it twice.

**A gift card names no owner, and the file says so.** `purchaser_customer_id` is
who paid; who spends it is whoever holds the code, and the schema's own comment
says as much: *"Often not the person who uses it."* The column is `purchaser`,
because calling it `customer` would be a plausible-looking wrong answer.
...
was checked, because `DATASETS` was a plain array. A dataset added to the union,
the labels and the switch but forgotten there would compile, be selectable by
name, and never appear in the default export. The labels are now the single
declaration; the union is `keyof typeof DATASET_LABELS` and the list is derived.
The blurb on the security page was a *fourth* copy — it still promised eight
datasets — and is generated too.

**Two defects the build itself turned up.** Going from nine files to twelve
showed that the browser was **silently dropping the last two**: `runExport`
revoked each object URL on the line after `click()`, racing the browser's fetch
of the blob, a race it won at nine and lost at twelve. The notice said twelve
because it counted `result.files.length` rather than what was delivered. Both
are fixed, and the count is now of what was actually saved. And `summarise` said
...
### The three answers to what was owed in March (Phase 108)

Every report comparing the ledger against the documents behind it walked the
**ledger** back to `asOf` and then read the **document** balance as it stands
now. Measured on the development books — three answers to one question, none of
them right:
...
any date but today — the register's highest severity, from a date picker on the
reports page. The aging report was wrong the other way, showing $1,241.94
outstanding in March because everything settled since reads as settled all along.
That is the third time in four phases a fault fired on a legitimate state.

**The reason it stood for seventy-seven phases was false.** `receivables-check.ts`
said reconstructing a historical document balance *"means replaying every payment
application, which is a bigger machine than this check justifies"*. Every one of
the four paths that reduces a balance writes a **dated** row — payments via
`payments.payment_date`, credits via `applied_on`, write-offs via
`written_off_on`, retainer draws via `applied_on`. It is four sums, not a replay.
Nothing enumerated the paths, which is why nobody noticed; `SETTLEMENT_PATHS`
declares them now, each with the column that dates it, and `pathFor` throws
rather than returning a silent zero. **I repeated that false claim myself in ADR
0107 the same morning; it is corrected there.**

**One query, shared by both readers.** `openDocumentsAsAt` feeds the aging report
*and* the control-account check. They each used to select open documents with
their own status filter — two answers to one question, which is where this whole
family of defects came from. Its filter is deliberately wide: a **paid** or
**written-off** document is included, because it was open before it was settled.

**The restored amounts convert at the document's own rate**, which is right
rather than approximate: a foreign invoice is carried at the rate it was raised
at until settlement, and the realised gain is a *separately dated* entry, so a
report from before that date must exclude it from both sides.

The result, on the same books:
...
**One thing it cannot recover, stated rather than hidden.** Voiding marks the
journal entry `void` and zeroes the document, dating neither — so an invoice
voided in July is absent from the March ledger *and* the March subledger. The two
still agree; both are wrong about March the same way. Dating the void is a change
to how correction works, not to how history is read.

### How far back a check can see (Phase 109)

Every check in the register takes an `asOf`. Most walk their **ledger** side back
to it and read their **subledger** side as it stands now — Phase 108 fixed the
two control accounts and did not reach the rest. Found by running all twenty-one
checks at three dates and looking for the ones whose answer moves:
...
The left figure never moves. It is a **fault**, so March reported $28,559.20 of
broken books on books that were correct — and `reconcileInventory`'s own comment
had already named the cost: *"a reconciliation that cries wolf is one people
learn to ignore."* Fourth phase in five to find a fault firing on a legitimate
state.

**Each check now declares how far back it can see**, with prose, on the device
Phases 70, 101, 105, 106 and 108 used. One that cannot answer for a past date is
...
is `value now − Σ(what moved since)` — one sum, no case analysis over the six
movement kinds. Declaring it `today_only` would have settled for less than the
data allows, on the very check the phase was found through. Afterwards, on the
same books: `2026-05-31: agrees 1668600/1668600`, `2026-03-31: agrees 0/0`.

**A date-gated skip is counted apart from a module-gated one.** A module switched
...
ADR 0110 named this and did not do it: **it does not repair the eleven**, with
`assets.register` and `manufacturing.wip` singled out as understood well enough
to fix. Verified before adopting it, by reading both queries and then measuring
four dates against the development books:
...
The left figure never moves — the signature Phase 109 found `inventory.lots` by.
Both are **faults**, so asking about last December reported $101,250 of broken
books and a broken factory floor on books that were perfectly correct. Both were
`today_only`, so nothing was lying; they were skipped, and eleven skipped checks
is eleven questions nobody can ask about a closed period.

**One shared decision is named; the shared arithmetic is not.** This is the
fourth subledger restored to a date, and the four walk back differently — the
control accounts *add* the settlements dated after, inventory *subtracts* what
moved after, assets *filter*, work in process *sums*. Collapsing those into one
function would need a flag saying which of four things it was doing, which is
four functions wearing a coat. What they share is one decision each of them
re-derived: **was this thing on the books that day at all**. `onBooksAt` and
`heldAt` name that and nothing else.

**Opening is inclusive and closing is exclusive**, which looks like an off-by-one
and is the opposite of one. Both dates are journal-entry dates — `registerAsset`
posts on `acquiredDate` and `disposeAsset` on `disposedOn`; `issueMaterial` posts
on `occurredOn` and `completeWorkOrder` on `completedOn` — and a report as at a
date includes every entry dated on or before it. Anything else puts the subledger
a day out of step with the ledger it is compared against, which is this phase's
own defect reintroduced at the boundary. Both edges are tested from both sides.

**The asset register** stops asking `status <> 'disposed'` and asks the two dates
instead. Depreciation was never the problem: `depreciation_entries` was filtered
by `period_end` all along, which is exactly why the check looked fine until
somebody read the other half.

**Work in process** was asking two present-tense things at once. `status =
'released'` is the one Phase 110 spotted; `wip_cents` is a running column and was
wrong even for a run still open, because it includes material issued after the
date. Both halves were already recorded — `work_order_entries.occurred_on` dates
every absorption with the same date its journal entry carries, and `completed_on`
is set, with an entry dated the same day, on cancellation as well as completion.

Afterwards, on the same books, every date agrees and both register sides walk
back to `0/0`. Twelve checks now reach any date; nine remain `today_only`.

**A shorter list says why it is shorter.** A past-dated register shows fewer
assets than today's, and without a word about it the reader has to work out
whether records went missing or were simply not there yet. The sentence rides on
the integrity check's `detail`, which the operations page already renders —
verified in the browser at 2026-02-28, where Ridgeline's register agrees at
$58,500 against today's $101,250 and adds *"1 asset is left out: it was not on
the books on 2026-02-28."*

### The figure a draw threw away (Phase 112)

ADR 0111 nominated `timebilling.retainers` as the clearest remaining candidate,
because `retainer_applications.applied_on` dates every draw. Reading the code
before adopting it turned out to matter: the nomination was right about the date
and wrong about the work. All three ways a retainer's balance moves *are* dated —
taken on `received_on`, drawn on `applied_on`, given back on a refund's
`refunded_on` — but **the draw does not record what it took**.

A draw works out what actually left the liability, in the company's own money,
posts it as the debit, and then wrote only `amount_cents` — the *client's*
currency — onto the row. And it is not derivable afterwards: `relieveFunctional`
gives the final draw the whole remaining functional balance so nothing is left
holding a stranded cent, which makes a draw's functional amount depend on the
balance at that moment — the very history being reconstructed. Circular.

`refunds.carried_cents` is the same fact about the *other* way a retainer goes
down, kept since Phase 68 in the same words. One of the two settlements of this
liability kept its figure and the other did not — **Phase 65's defect for the
sixth time**, a functional amount computed and dropped.

**Measured on honest data.** The development books held only a retainer inserted
by raw SQL in Phase 105 for a browser demonstration, with no journal entry behind
it — so it was removed and replaced with one taken and drawn through the service:
...
The held figure never moves, and this is a **fault**: $5,000 of client money read
as broken books in May, and $3,200 read as held a month before the client had
sent anything.

**The column is kept, and backfilled exactly.** `retainer_applications` gains
`carried_cents`, and the migration reads each existing row's figure back off the
debit its own journal entry posted — the truth, not a reconstruction. It joins on
the account the entry touched rather than a chart number, because a company on a
pack without 2550 holds retainers on 2500 and both are legitimate.

**The balance is rebuilt, not read off the running column.** `heldByAt` assembles
a retainer's dated life and hands it to Phase 111's `heldAt`. That reuse is the
point: *was this on the books, and what did it hold* is the same question a fifth
time, named once so this phase did not decide it again. A retainer has **no
closing date** — it does not leave the books when it reaches zero, it simply
holds nothing — so the lifespan is open at the far end.

Afterwards every date agrees and the held figure walks back to `0/0`. Thirteen checks
reach any date; eight remain `today_only`.

**The check got stronger as a side effect.** Every legitimate reduction of a
retainer writes a dated row — `applyRetainer` an application, `refundRetainer` a
refund, and there is no third path. So a balance that came down with nothing
recording where the money went now reads as *still held*, where before the
subledger simply agreed with itself. Two existing tests asserted the old
behaviour using rows no code path can produce; both were rewritten, and the new
behaviour has a test of its own.

### The credit spent in July (Phase 113)

ADR 0112 said that repairing `receivables.customer_credit` "would mean the same
remedy, a dated application row carrying what it took — a bigger change than a
column, because **those rows do not exist at all**."
...
a `created_at`. Another impossibility asserted without looking, in the phase
straight after one whose whole subject was impossibilities asserted without
looking.

The consequence is far worse than an integrity check's reach, because **two
readers need that date and both invented one from `payments.payment_date`.**

**The cash-basis report misstates a period and mutates a closed one.** This
codebase recognises cash-basis revenue *through the document a payment settles* —
its own caveat says so — so an application belongs to the period it was made in.
Held credit spent months later is where the two dates part company. Measured by
running the phase's tests against the code as it stood:
...
A $5,000 overpayment in March against a $3,000 invoice, the $2,000 applied to a
July invoice: **July's cash-basis revenue was zero**, and **March's grew from
$3,000 to $5,000 after the fact**. A period that may have been closed, filed and
reported on returns a different number when it is re-run.

**Phase 108's machinery had the same substitution.** `settlementsAfter`, which
walks the receivables control account back to a date and which Phases 108–112 all
build on, dated an application by the payment too — so asked about June it undid
a July application, putting back a balance that had already been settled.

**The backfill reproduces the past rather than inventing it.** Existing rows take
`payments.payment_date`, deliberately. The true date of a credit applied in the
past was never written down anywhere: `applyCredit`'s journal entry carries
`source_type = 'payment'` and the payment's own id, exactly like the payment's
original entry, so nothing separates them but a memo string. Parsing memos would
silently restate periods that have already been reported on. The payment date is
what every reader has been using, so the backfill leaves historical reports
saying what they have been saying, and new applications carry the truth.

### The credit spent at a rate it was never carried at (Phase 114)

ADR 0113 nominated a column so `receivables.customer_credit` could be promoted
to `any_date`. Verifying that before adopting it found something worse in the
way: **the check could not have been promoted, because the settlement it checks
was wrong.**

`customer-credit.ts` settles the same liability in two places and only one of
them was right. `refundCredit` — giving held money back — uses Phase 68's
`settleHeld`: it relieves the held balance at the rate the money came in at, the
other side at its own rate, and posts the difference as a realised gain or loss.
`applyCredit` — spending the credit against an invoice — converted **both** sides
at the invoice's rate for the ledger, while the subledger column beside it came
down at the **payment's**. Two rates, one movement. And they need not be
different currencies to differ: a euro receipt in January and a euro invoice in
June are the same currency at two rates.

**Measured.** €5,000 arrives in January against a €3,000 invoice at 1.10,
leaving €2,000 held and carried at **$2,200**. In June, at 1.25, it is applied to
a €2,000 invoice carried at **$2,500**:
...
Three consequences of one substitution: `2520 Customer Overpayments` left
holding **−$300** — a liability with a debit balance, because the entry took out
$2,500 that was never put in; **the fault-severity check firing**, subledger
against ledger; and **the $300 never recognised anywhere**, because nothing ever
asked for an FX account.
...
does a few hundred lines below it in the same file. Two smaller things fell out
of doing it properly, both the same defect in miniature: the invoice's functional
balance was recomputed by conversion rather than taken from the relief — a second
answer to what the invoice gives up, losing the rule that stops a stranded cent —
and `functionalUnappliedCents` called `relieveFunctional` a second time with
identical arguments, where it now reuses the number the entry actually debits.

**What I got wrong on the way.** My first draft of the test asserted the realised
loss as +$300, on the grounds that a loss is a debit. `7100` is typed
`other_income`, so `balanceForAccount` signs it in the account's normal
direction and a debit reads as −$300. The implementation was right and my
assertion was wrong; the test now says so in a comment.

### The check that compared euros with dollars (Phase 115)

ADR 0114's nomination went the same way, and this time the thing in the way was
**the check itself**.

`heldCredits` selected the held amount in the payment's own currency, and
`receivables.customer_credit` added those rows up and compared the total against
`balanceForAccount(2520)` — which is functional, because every journal line in
this system is. A euro overpayment put **€2,000 against $2,200** and reported
broken books on correct ones. It is a **fault**, not a position, and the check is
ungated: every company runs it nightly, so a business that took one overpayment
in a currency other than its own got a red report for ever on books that were
right.
...
phase for exactly this. Phase 114's tests missed it because they applied the
credit **in full**: both sides came to zero, and zero is zero in any currency.

**Measured.** €5,000 received in January against a €3,000 invoice at 1.10,
leaving €2,000 held and carried at $2,200:
...
`heldCredits` now carries the currency and the functional figure beside the face
one, and the check sums the functional. **The split is the point.** A person
reading a list of held credits wants the figure their customer would recognise
on their own statement — €2,000, not "about $2,200". A *total* cannot be that
figure, because two rows may be in two currencies and their sum would be a
number in no currency at all. Both are right; they answer different questions.

**The same defect three more times, on the same screen.** Verifying the repair in
the browser showed the payments screen rendering a €5,000 receipt as
`$5,000.00` — `formatCents` has taken a currency argument since Phase 35 and
this screen passed it nowhere. Each row now renders in its own currency; the
RECEIVED and PAID OUT tiles sum a functional amount converted once in
`listPayments`, at the rate fixed when the money moved rather than today's, which
would restate what the business banked every time a currency moved. On the
seeded books the received total moved from $37,668.00 to **$38,568.00** — two
euro receipts that had been counted at face value. The void panel's restoration
lines take the **document's** currency, not the payment's, since
`payment_applications` records what the document was relieved by.

**What it cost.** A test that tampered with `unapplied_cents` alone to prove the
check notices a subledger moved behind the ledger's back no longer catches
anything — correctly, because the functional column still agrees with the ledger
and the books really are consistent. What is broken there is the relationship
between a payment's own two columns, which is a different question. The test was
rewritten to move both, and a second was added recording the gap explicitly
rather than leaving somebody to find it. ADR 0115 nominates the check that would
...
- **`banking.shared_ledger_accounts` cannot be falsified at all**, and that is
  ...
  `NOT_YET_PROVEN` with the question written down. **Phase 122 settled it: the
  constraint's own migration repaired those books, so it is the first case, and
  the check is retired.**
...
### The sums that still added currencies (Phase 122)

Phase 65 closed *"the three sums that still add currencies"*. Phase 115 found
the integrity register doing it. Phase 116 gave every face amount a functional
twin and a constraint keeping the pair honest. Three phases across fifty, each
closing instances of one defect — and nothing had ever asked how many were left.

Asking it, over every `sum()` of a face-amount column in `src/modules`:
...
**Two of the eight decided money rather than describing it.**
`vendorCreditBalances` totals what each supplier owes back and the pay run nets
that against what is owed them — so a €500 credit and a $500 credit made "1000"
of nothing, and that number came off a payment. It is keyed by supplier **and
currency** now. `assistants.ts` computed revenue concentration, the largest
client's share, by adding invoice totals across currencies and dividing; it
reads the functional twin. The rest report: two deactivation refusals (now
functional), two till takings (argued safe), and one cash-basis sum that was
computed and never read, which is deleted rather than repaired.

`src/modules/fx/comparable.ts` names the nine face columns and the rule — group
by currency, sum the functional twin, or argue from the code that the rows are
provably one currency — and `tests/comparable-sums.test.ts` reads the source and
enforces it. The two excused sums are both in the cash drawer, and the excuse is
**verified in the code rather than argued from what a till is like**:
`takeCounterPayment` never passes a currency to `recordPayment`.

`payments.amount_cents` is the trap in four of the eight sites: it has no
functional twin at all, so the only way to make it comparable is `convert()` at
read time. That is why the list is written out rather than derived from
`PAIRED_COLUMNS`.

**`banking.shared_ledger_accounts` is retired**, which settles the question
Phase 121 recorded. The check and `financial_accounts_chart_account_unique`
arrived in the *same commit*, and that migration repairs every existing sharing
pair before adding the constraint — so the "migrated books" the check existed
for were repaired by the migration that installed it. It has been unable to find
anything since it was written, and ran nightly on every company saying so. **A
constraint beats a check** (Phase 116), and here the constraint arrived first.
`sharedLedgerAccounts` goes with it: the register was its only caller in 82
phases. Its three tests are replaced by one that asserts the constraint refuses
the insert — the previous best test had to `DROP CONSTRAINT` inside a
rolled-back transaction to give the check anything to find, which is the check
telling you the state cannot occur.
...
### The currency one join away (Phase 131)

ADR 0130 nominated the write-off and deposit residues. Measuring it turned up
something better: **the scan that decides whether a screen serves foreign money
was asking a list of seven tables typed by hand, and the schema has thirteen.**
That is Phase 128's defect one file over — that phase replaced a hand-typed list
of nine in the posting scan and never looked at its sibling. The prose was worse
than the list: `on-screen.ts` and its test both said `deposits` carries no
currency column, and it has since Phase 127 added one in the same migration as
the write-offs ADR 0130 was pointing at. Phase 110's failure again — a
declaration argued from a schema fact that is not a fact.

**But widening the list does not fix it, and that is the phase.** `bank_transactions`
has no currency column and never will; it takes the account's. Phase 128 wrote
exactly that down in prose and made no rule of it, so three screens were outside
the scan by construction and had shown a foreign account's money with a dollar
sign since Phase 1: the transaction inbox, the reconciliation workspace, and the
mobile review deck.

`INHERITED_CURRENCY` is the rule that sentence should have been. The test is a
**mandatory** foreign key to a currency carrier — a nullable one is a link, not a
denomination, which is why a time entry's rate is the company's money whether or
not it is ever billed. Thirteen tables qualify, asked of `information_schema`,
and each splits its `%_cents` columns into the parent's money and the books'
against every such column the table actually has. Three answer *the books'* —
inventory cost frozen onto an invoice, a remittance measured against a ledger
balance, and the functional twin Phase 129 put beside a bank transaction — and
those are the entries that make it a registry rather than a list.

Nothing about the arithmetic changed. A reconciliation session is self-consistent
in the bank's money and `summarize` converts nothing; every figure was correct and
unlabelled, which is the failure Phase 124 exists to stop and the one it could not
reach. `ReconciliationSummary` carries the account's currency now rather than the
page looking it up beside the figures.

**The scan sees through `Math.abs`.** The deck writes
`formatCents(Math.abs(current.amountCents))` and the check matched the bare form
only, so widening the tables alone would have brought the deck into reach and
still passed it. Thirteen call sites in `src/app` wrap money that way. Writing
that unwrap immediately caught its own overreach — the new branch matched the
*repaired* call too, because it stopped at the inner bracket.

Browser-verified against a euro account on seeded data: the inbox reads €300.00,
€800.00, €500.00 and shows three euro rows beside seventeen dollar ones in the
same list; a reconciliation on that account reads €0.00 opening, €1,000.00
statement, €1,000.00 difference with all three transactions in euro; and the deck
reads €500.00.

The unclassified remainder Phase 124 built moves from 12 to 21 — and that growth
is the thing being said why for. Nine more pages came into reach (budgets, the
asset register, dimensions, proposals, the chart of accounts, the takings board)
and every one shows the company's own money, reached only because a page imports
a module that touches `bank_transactions` somewhere else. Counting them beats
...
code. §19 asks for four things — *"Backups, point-in-time recovery strategy,
retention policy, and tested restore procedure"* — and only the retention policy
existed. `docs/DEPLOY.md` ran to 323 lines and contained no occurrence of
*backup*, *restore* or *recovery*.

The finding is not the missing section. It is that a database backup is not a
backup of this system, and the argument was already here, written from the other
side. `secret-box.ts` says a leaked dump is safe because *"the key was never in
it"* — which is true, and is exactly why a dump you **keep** is insufficient.
Restore without `ENCRYPTION_KEY` and every MFA-enrolled user is locked out of
their own books permanently: AES-GCM authenticates, so a wrong key fails rather
than producing a plausible secret. The row does not become wrong, it stops being
data. One file held both halves of that and neither mentioned a restore.

Six things have to come back, and the distinction that matters is whether you
could change the value **on purpose on a Tuesday**:

| target | regenerable |
| --- | --- |
| the database | no |
| document bytes under `OBJECT_STORE_PATH` | no |
| `ENCRYPTION_KEY` | **no** — the ciphertext dies with it |
| `VAPID_PRIVATE_KEY` | **no** — every push subscription dies with it |
| `SESSION_SECRET` | yes — everybody signs in again |
| `CRON_SECRET` | yes — set it at both ends |

The last two are on the list *because* they do not matter: a register whose
entries are all emergencies is read as one undifferentiated emergency, and
`restoreStands` does not hold a restore open for either.

Whether anything lives outside the database is a fact **in** the database.
`document_blobs.storage_provider` records the adapter per row and its own comment
already insisted on it — "Read from here, never from the setting" — so
`storesInUse` is measured rather than declared (ADR 0141), and a company that
never left the database adapter is not sent looking for a directory.

The claim that `ENCRYPTION_KEY` cannot be regenerated is the whole argument, so
the test performs it rather than asserting it: encrypt, decrypt, swap the key,
watch it throw. The rehearsal in DEPLOY.md has the same shape — step 3 is *sign
in as a user with MFA enrolled*, called out as the step that catches a missing
key because it is the only one that does. Step 4 sends the operator to
**Settings → Integrity**, because twenty-odd checks that prove the subledgers
still agree with the control accounts already exist, and writing a second set
would have been the defect this project keeps naming.

It caught two faults in itself, both the same fault: the query said `store` and
the column is `storage_provider`, and the environment check looked in a list of
four files I typed and failed on `SESSION_SECRET`. Both were found because the
test runs against the live schema and scans the tree.
...
money-versus-money comparisons in `src/modules`, 30 in files reading a
currency-bearing table, across 23 functions** — and nothing scanned any of them.
...
Four forms, and the fourth is the one that mattered. A relational operator,
`Math.min`/`Math.max` and an equality are what anybody would write down, and
between them they reach Phase 138's `applyDeposit`. They do **not** reach Phase
142's `redeemGiftCard`, whose comparison lives inside `redeemFor` — a pure helper
in a file that reads no currency-bearing table, over two bare parameters. So the
fourth form is the call itself: **a helper handed two money amounts cannot know
whether they are comparable.** Stopping at three would have built a scan that
missed the defect it was for.

Five reasons two amounts can be compared, declared per site and measured against
the source:
...
result rather than a disappointment: a new tripwire announcing three defects
nobody has heard of is one nobody can check. This one disagrees on its first run
and what it disagrees about is every comparison defect this project has found by
hand, and nothing else. `contractorPayments` is now caught from both ends — Phase
143 registered the sum, this registers the comparison of that sum against a
statutory threshold.

Why it is per site: `applyDeposit` and `refundDeposit` contain the **same
expression**, `input.amountCents > position.heldCents`, and one is wrong.
Refunding hands a tenant back the company's own money; applying it puts that
money against an invoice that may be in any currency at all.

One rule the scanners were missing turned up here. The first run reported three
extra sites, all registries quoting real comparisons inside their prose — one of
them **the entry this phase itself wrote**. Phase 141 established that a posting
site is code and a sentence about one is not, and blanked comments; **string
literals are the other half**.

### The money columns nobody declared (Phase 143)

ADR 0142 ended by saying nothing new was found. Verifying that rather than
believing it started somewhere else — two of the last five defects were money
**compared** with money rather than summed (Phase 138's `>` and Phase 142's
`min`), both found by hand, and `ADDITION_FORMS` covers only addition. Building a
comparison scan meant asking what it would key on, and that is where the finding
was.

`FACE_COLUMNS` — the list both sum scans key on — was **seventeen column names
typed by hand**. Asked of `information_schema`, the carrier tables have
**fifty-four** money columns. The thirty-seven nobody classified were not excused
by the tripwire that says "no sum adds two currencies together"; they were
**invisible** to it, which is worse. An excused site carries an argument somebody
can disagree with; an unseen one carries nothing and the scan reports all clear.

This is Phase 128's shape exactly — that phase made `CURRENCY_CARRIERS`
answerable to `information_schema` after a hand-typed list of nine missed four
tables and took twenty-two posting sites out of reach. `FACE_COLUMNS` sat in the
same module with the same failure mode and never got the same treatment. Seventh
reach failure, after 128, 131, 133, 136, 140 and 141.

Completing the list surfaced five sites. Measuring each — rather than counting
flags — found **three real and two the scan's own narrowing got wrong**:

| | |
| --- | --- |
| `contractorPayments` | sums `payment_applications.amount_cents` and compares it against a statutory threshold in home money — **a 1099 filed on arithmetic over incomparable things** |
| `salesTaxReturn` | sums `invoices.subtotal_cents` for **a return filed with a tax authority** |
| `cashBasisCaveats` | sums `invoices.tax_cents`; describes rather than decides |
| `openCreditsAsAt` | **not a defect** — grouped by `credit_note_id`, and a note has one currency |
| `previewBilling` | **not a defect** — the reduce is over `time_entries`, which has no currency column |

Three sides rather than one, so the completed list makes the scans wider instead
of noisier: `face` (44) is the document's own currency and the only side the
scans ask about; `functional` (5) is already the company's money — a refund's
`carried_cents` says so in its own schema comment; `account` (5) is one account's
arithmetic end to end, which `cashTieOut` already argues.

The three live defects go in **`BLIND_FACE_SUMS`**, which **indicts** rather than
excuses — the opposite of `SAFE_FACE_SUMS`, and a test asserts no site is in
both. Putting them in the excuse list would have been ADR 0134's failure: a
declaration that excuses a site is worse than one that misses it. Each names its
defect and points at a skipped acceptance test.

They are registered rather than repaired because the staging pass is holding live
service paths. Each is a query change and available on request; leaving them
undeclared was the only unacceptable option.
...
`domestic` — "the money cannot be foreign here, argued from the schema". Each of
the fourteen argues. None of them said **what kind of argument** it was making,
and the kinds are not interchangeable: nothing with a currency is in reach; the
path refuses when one differs; a callee converted it first; this function writes
it at a rate of one; the figure is a sum already argued to be one currency.

So the ground is now declared and the reach is measured. Choosing the argument is
a judgement a person makes; remembering what a function reaches is not, and it is
the half that goes stale — which is ADR 0134's rule applied to the half that could
excuse a site.

Three of the fourteen argue a ground the source contradicts, and each keeps the
argument it has always made so the scan disagrees with it by name:

| entry | argues | and the source says |
| --- | --- | --- |
| `applyDeposit` | "the lease carries no currency either" | reaches `invoices` through `settleInvoiceWithoutCash` |
| `redeemGiftCard` | "a gift-card balance has no currency column" | reads `invoices` itself |
| `recordContribution` | "`contributions` and `funds` carry no currency column" | debits a bank account read straight out of the table |

`redeemGiftCard` bounds a home-currency card by the invoice's **face** balance,
posts that to both lines, then relieves the invoice through `relieveFunctional`,
which converts — so on a euro invoice the control account and the subledger move
by different amounts. Phase 137's defect, in a path Phase 137 did not reach.

`recordContribution` is the **sixth** instance of this codebase's recurring reach
failure. Phase 133's scan matches `chartAccountId: bank.chartAccountId` or a name
containing `gl`; this assigns to `debitAccountId` first, so it is absent from
`BANK_POSTINGS` entirely — a scan that looked for a spelling rather than for the
fact. Its sibling `receivePledge`, forty lines below in the same file, goes
through the gate. Same business, same account: told no for a pledge, nothing at
all for a gift. Measured across all ten reads of
`financial_accounts.chart_account_id`: four go through the gate, five post
nothing, and one is this.

One structural property closes the hole a self-declared field would leave:
`covered` is built only from what a named callee reaches, so a carrier a function
reads itself can never be excused by one. Relabelling a failing entry
`converted-downstream` and pointing at something plausible is refused, and there
is a unit test that tries it.

Two entries are more interesting than they look. `closeShift` reaches `payments`
and is right anyway — it points at `SAFE_FACE_SUMS` rather than restating that
argument. `commitOpenDocumentImport` reaches four carriers because it *creates*
them, and its ground checks that the face column and its functional twin take the
**same expression**, which is what "the rate is one" looks like written down.

The scanners needed a guard of their own: `fx/ground.ts` quotes the pattern
`bank-side` looks for while explaining it, and the scan reported its own
documentation as a twentieth posting. All four now blank comments before
matching — a posting site is code, a sentence about one is not — using Phase 140's
offset-preserving reader so line numbers still belong to the file.
...
named as still open in **ADRs 0131, 0132, 0134 and 0135** — the last two
consecutive, which Phase 31 and 33 established usually means it *is* the phase.

Measured, `mayPostToBank` took the account's currency and the home currency and
**never saw the money**. So one refusal covered two situations that are not the
same:

- **the money and the account agree** — the bank feed's shape, correct since
  Phase 128 because `bank_transactions` inherits its currency from
  `financial_accounts`. €96.80 really landed, the statement says so, the ledger
  takes the converted figure. Nothing is unknown.
- **they disagree** — the **bank** converted, at its own rate, and these books do
  not have it. Any figure posted is a guess at somebody else's arithmetic, and
  Phase 40's tie-out compares each account in its own currency, so the statement
  and the ledger differ by the spread with nothing to name it.

**Phase 133 refused the first and Phase 134 converted the second** — exactly
backwards. The new tests found it in Phase 134's own database test, where three
assertions written against a dollar account passed: self-consistent arithmetic
describing something a bank would not do. They are repaired onto a euro account
rather than deleted, and the mismatched case is now its own refusal.

`BANK_POSTINGS` gains `matched`, argued rather than bent from a neighbour (Phase
130): `converts` would claim it always posts, `refuses` that it never does, and
both are false half the time.

Phase 135's prose check did **not** fire on the newly stale entry, correctly: it
catches denials of conversion, and this was a claim about what is refused. ADR
0135 said exactly that limit, so it is confirmed rather than discovered.

**Part 3 corrected this document's own claim.** It said `importPayouts` was the
only one of Phase 133's ten with a field saying what currency the money is in and
that "the other nine have no such field" — written from memory, one ADR after the
phase built to catch exactly that. Measured by reading each body, **five of the
nine already had it**, every one already reading it for a rate lookup and simply
never handing it to the guard: `receiveRetainer`, `refundRetainer`,
`refundCredit`, `refundVendorCredit` and `recoverWriteOff`. Only four —
`receivePledge`, `receiveDeposit`, `refundDeposit`, `recordRemittance` — have
nothing.

Four are now wired, and the fifth is the interesting one. **Having the currency
is not enough to be told it**: the figure a path puts on the bank must be struck
at the rate on the day the money moved, or letting it post buys an entry no
statement can be tied to. `recoverWriteOff` posts `recovery.functionalCents` to
*both* lines at the write-off's carried rate — right for bad debt, wrong for the
bank, one figure answering two questions — so it keeps refusing and
...
that the source contradicts. Three measured counts moved across the phase and all
three were caught by their own devices — refuses ten → nine → **five**, and
**Phase 135's** six → five → **one**.

The new scan found its own bug first: its paren-walker read the apostrophe in
"at the day's rate" as opening a string literal, ran off the end of the function
and reported *no* arguments, so two correctly wired sites looked unwired. A scan
going quiet on prose it did not expect is the Phase 128 / 131 / 133 failure one
layer down, fixed in the walker rather than by rewording the comments.
...
refusals. Measuring it showed the nomination understated the defect, and the
correction is recorded in ADR 0133 rather than claimed here: `batch.currency`
was written to `payouts.currency` and **never read again**, so a euro payout
into a *domestic* account posted €X as $X — which Phase 133's guard cannot
catch, because it asks about the account and never about the money.

Phase 44 built `1250 Payments in Transit` on three lines. Measured, they were
not in the same money. `recordPayment` debits a **converted** figure; `postFee`
credited the face fee and `importPayouts` the face payout. A €100 charge with a
€5 fee at 1.10 leaves **$10.00** in the account that is not a balance, not a fee
and not a gain, and no report names it.

Both offending sites were declared `domestic` in `LEDGER_POSTINGS`, and both
arguments end the same way — "a fact about the data rather than about the
schema". Both were corrected in Phase 128, and both corrections fixed the
*description* of where the currency lives while leaving the *posting*
unconverted. This is Phase 110 and 125's class at its most legible: a
declaration arguing from a fact it **admits is not guaranteed**. The new columns
are those hedges turned into facts about the schema.

The bank now takes the arrival rate, the clearing account is relieved of exactly
what it was charged at the capture rate — an account relieved at another rate
can never reach zero, which is the only thing a clearing account is for — and
the gap is a realised exchange gain, Phase 67's rule for retainers applied to
money a processor holds. `clearedCents` sums what the earlier entries actually
posted rather than recomputing: three €3.33 charges convert to 1098 where their
€9.99 total converts to 1099, and recomputing would leave a correct batch a cent
adrift, indistinguishable from a real discrepancy.

Underneath it, a **fourth** instance of the scan-reach failure. Phase 122's
`currencyAware` read a fixed forty-line window and excused `heldByProcessor` —
which sums `gross_cents - fee_cents` with no currency in it — because
`recentCheckouts`, a different function twenty-four lines below, selects
`currency:`. Phases 128, 131 and 133 each found a scan that *missed* sites; this
one **excused** one, which is worse, because a miss leaves an absence and an
excuse reports all clear. Bounded by the enclosing function it catches that site
and only that site. `payoutReconciliation` was then retired as Phase 49's class,
its cases ported onto `payoutSettlement`.
...
Verified in the browser: `/properties` → Deposits held offers Ridgeline's euro
account like any other, and taking a deposit into it puts the refusal on screen
word for word while `deposit_movements` keeps only its seeded row and the
register still agrees with account 2580. The confirming suite went red on
`registry-error.test.ts` — Phase 132's measured count of eleven `RegistryError`
throws caught `BANK_POSTINGS`, written a phase later by somebody who never
opened that test. The first time that device has caught a registry rather than
described one.
...
| `tests/integrity-reach-proof.test.ts` | **Proving what Phase 109 only claimed**: a check whose subledger side honours the date reports *nothing* for `1900-01-01`, before any company in this application existed — held against a real invoice (100000 today, 0 then) and real stock (5000 today, 0 then), because the test is vacuous where the fixture has no activity and that is said in the comment rather than counted as evidence; the ungated `any_date` checks swept the same way, with the module-gated ones left out because the register skips them for wanting a chart this company has no reason to carry; and what verifying the fifteen declarations changed — **three promoted** (`properties.deposits` on `occurred_on`, `pos.tips` on `business_date`, `funds.untagged_contributions` through `netAssets`), **two kept** for reasons only the query shows (`assets.register`'s undated `fixed_assets`, `manufacturing.wip`'s present-tense `status = 'released'`), the seven with no dated history left alone, and **no declaration anywhere still saying "not verified"** — with the two Phase 111 went on to repair now asserted `any_date`, their old reasons gone from the register, and the 1900 proof extended to the asset register so three ungated checks carry weight rather than two |
| `tests/lifespan.test.ts` | **Opening inclusive and closing exclusive, asserted from both sides of both edges** — on the books the day it arrives and not the day before, off the books the day it leaves and not the day after, and a thing bought and sold on one day on the books for no report at all; a null opening date meaning *never on the books* rather than the beginning of time, so a draft work order stays off every historical report; **a lifespan that closed before it opened throwing rather than answering**, because both silent answers are wrong half the time on a report somebody reconciles against; what had been absorbed by a date and nothing after it; a holding worth zero once closed whatever its movements say, and still worth them the day before; and a set of holdings returning which ones counted as well as the total, since *which ones* is the first question after a difference |
| `tests/subledgers-as-at.test.ts` | Against the database, the two subledgers Phase 111 repaired: **an asset bought in June not counted in a March report** (5000000, not 5800000, against a ledger walked back to the same), counted on the very day it arrives, still counted in March when it was sold in June, and gone on the day of sale rather than the day after; accumulated depreciation still agreeing, so the half that always worked is shown not to have broken; **a run released in February and finished in May still on a March report** — the case that made it `today_only` — holding only what had been issued by then rather than the running `wip_cents`, holding nothing on the day it finishes, treating a cancelled run the same way as a finished one because `completed_on` means the same thing for both, and a draft run left out of every date from 2020 to 2030; both answering the same undated as for today, so the nightly run is unchanged; both declarations reading `any_date` with the columns that make it possible; and **the exclusion reaching the check somebody actually reads**, on the `detail` the operations page renders |
| `tests/retainer-as-at.test.ts` | **Client money stated as at a date** (Phase 112): a retainer holding nothing before it arrived and the whole amount from the day it did, coming down on the day of a draw rather than the day after, coming down again when some is given back, and summed correctly however the rows happen to come back — a date filter is not a sort; across a firm, **the total and the open count from one walk**, since "how much are we holding" and "on how many retainers" are one question asked twice; and against the database, **both sides walking back together** through 0 → 500000 → 320000, an open count that falls to zero before the money arrived, a refund landing where the ledger puts it, the same answer undated as for today, **the functional figure a draw works out now kept on the row**, and a euro retainer held at 110000 rather than the 100000 that arrived |
| `tests/credit-applied-later.test.ts` | **A March overpayment applied to a July invoice** (Phase 113): the July work reported in July rather than nowhere — it was `+0` before — and **March left exactly as it was**, which is the half that matters because March may be closed, filed and reported on; the everyday case asserted alongside, where an application made on the day of payment still lands in that day's period and nothing about the answer changes; and the year covering both holding $5,000 of revenue against $5,000 of cash, so a repair that moved money into the right month without preserving the total would be caught |
