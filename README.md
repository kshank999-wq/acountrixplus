...
- **The verdict has no memory.** The window is a fixed seven days and nothing writes the number
  down, so 3% today cannot be told from the fourth week of a slide — and those call for different
  actions. The one question a reputation metric exists to answer, *is this getting better or
  worse?*, is the one the page cannot answer.
